...
const currPath = './src/data/curriculum.ts';
let fileContent = fs.readFileSync(currPath, 'utf8');

const match = fileContent.match(/export const curriculumData: Lecture\[\] = (\[[\s\S]*\])\s*\.sort/);
if (!match) {
...
const part1Content = week8.content.slice(0, idx3);
const part2Content = week8.content.slice(idx3);

const w8 = {
  week: 8,
  stage: 5,
  stageTitle: "현대 비평 본문 비판 (상)",
  title: "현대 비평 본문 비판",
  instructor: "",
...
const newFileContent = fileContent.substring(0, match.index) + 
  "export const curriculumData: Lecture[] = " + newArrayStr + "\n.sort((a, b) => a.week - b.week);\n";
