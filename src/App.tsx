...
export default function App() {
  ...
  const handleNext = () => {
    if (currentWeek !== null && currentWeek < 15) {
          ...
          {currentWeek !== null && (
            <div className="hidden md:flex items-center gap-8">
              <div className="flex flex-col items-end gap-1">
                ...
                <span className="text-sm font-black text-slate-900 tabular-nums">{Math.round((currentWeek / 15) * 100)}%</span>
              </div>
              <div className="w-48 h-1.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200/30">
                <motion.div 
                  initial={{ width: 0 }}
                  animate={{ width: `${(currentWeek / 15) * 100}%` }}
